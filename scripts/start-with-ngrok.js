...
function updateEnvWebhook(url) {
    ...
    const next = lines.map((line) => {
        if (line.startsWith('TELEGRAM_WEBHOOK_URL=') || line.startsWith('# TELEGRAM_WEBHOOK_URL=')) {
            found = true;
            return `TELEGRAM_WEBHOOK_URL=${url}`;
        }
        if (line.startsWith('LINE_WEBHOOK_URL=') || line.startsWith('# LINE_WEBHOOK_URL=')) {
            return `LINE_WEBHOOK_URL=${url}`;
        }
        return line;
    });
    if (!found) {
        next.push(`TELEGRAM_WEBHOOK_URL=${url}`);
    }
...
async function run() {
    ...
    logger.info('Updated TELEGRAM_WEBHOOK_URL in .env');
    process.env.TELEGRAM_WEBHOOK_URL = url;
    process.env.LINE_WEBHOOK_URL = url;
