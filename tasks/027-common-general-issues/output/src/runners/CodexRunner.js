...
class CodexRunner extends BaseRunner {
    ...
    _spawnCodex(args, outputFile, workdir, context = {}) {
        return new Promise((resolve, reject) => {
            let stdout = '';
            let stderr = '';
            let sessionId = null;
            const { appendEvent } = require('../utils/live-stream');
            const token = context.token || 'default';
            const userMessage = context.prompt || '';
            let userEventSent = false;
            ...
            const maybeEmitUserEvent = () => {
                if (userEventSent || !sessionId || !userMessage) return;
            ...
            child.stdout.on('data', (data) => {
                ...
                maybeEmitUserEvent();
                this._emitCodexEvents(text, token, sessionId, appendEvent);
            });
    ...
    _emitCodexEvents(text, token, sessionId, appendEvent) {
        ...
        lines.forEach((line) => {
            ...
            const mapped = this._mapCodexEvent(event);
            if (!mapped) return;
            const eventSessionId = event.thread_id || event.session_id || sessionId;
            appendEvent(eventSessionId, {
                ts: new Date().toISOString(),
                token,
    ...
    _mapCodexEvent(event) {
        if (event.type === 'item.completed' && event.item) {
            if (event.item.type === 'agent_message') {
                return { type: 'message', text: event.item.text || '' };
            }
            if (event.item.type === 'reasoning') {
                return { type: 'status', text: event.item.text || '' };
            }
            if (event.item.type === 'command_execution') {
                return {
                    type: 'tool',
                    text: `Explored: ${event.item.command || ''}`,
                    meta: { command: event.item.command, output: event.item.aggregated_output }
                };
            }
        }
        if (event.type === 'thread.started' && event.thread_id) {
            return { type: 'status', text: `Thread started: ${event.thread_id}` };
        }
        return null;
    }
