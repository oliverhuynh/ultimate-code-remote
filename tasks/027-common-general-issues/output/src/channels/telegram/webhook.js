...
class TelegramWebhookHandler {
    ...
    async _handleMessage(message) {
        ...
        if (messageText.startsWith('/live')) {
            const match = messageText.match(/^\/live\s+([A-Z0-9]{6,8})$/i);
            const token = match ? match[1].toUpperCase() : null;
            if (!token) {
                await this._sendMessage(chatId, 'Usage: /live <TOKEN>');
                return;
            }
            ...
            if (!baseUrl) {
                await this._sendMessage(chatId, 'Live stream base URL is not configured.');
                return;
            }
            const secret = issueLiveSecret(token);
            const url = `${baseUrl}/tokens/${token}/live/view?pw=${secret.liveSecret}`;
            await this._sendMessage(chatId, `🔗 Live stream: ${url}`);
            return;
        }
    ...
    async _sendHelpMessage(chatId) {
        const message = `📚 *AI Code Remote Bot Help*\n\n` +
            ...
            `• \`/live <TOKEN>\` - Get live stream URL\n\n` +
    ...
    async setWebhook(webhookUrl) {
        try {
            ...
            this.logger.error('Failed to set webhook:', detail);
            throw error;
        }
    }
