...
class LINEWebhookHandler {
    ...
    async _handleTextMessage(event) {
        ...
        if (messageText.startsWith('live') || messageText.startsWith('/live')) {
            const match = messageText.match(/^\/?live\s+([A-Z0-9]{6,8})$/i);
            const token = match ? match[1].toUpperCase() : null;
            if (!token) {
                await this._replyMessage(replyToken, 'Usage: /live <TOKEN>');
                return;
            }
            ...
            if (!baseUrl) {
                await this._replyMessage(replyToken, 'Live stream base URL is not configured.');
                return;
            }
            const secret = issueLiveSecret(token);
            const url = `${baseUrl}/tokens/${token}/live/view?pw=${secret.liveSecret}`;
            await this._replyMessage(replyToken, `🔗 Live stream: ${url}`);
            return;
        }
    ...
    async _replyMessage(replyToken, text) {
        try {
            ...
            this.logger.error('Failed to reply message:', detail);
        }
    }
