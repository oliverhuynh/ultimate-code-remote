...
class CommandRelayService extends EventEmitter {
    ...
    _queueCommand(commandData) {
        ...
        this._appendLiveEvent(queueItem, {
            type: 'user',
            text: queueItem.command
        });
        this._appendLiveEvent(queueItem, {
            type: 'status',
            text: 'Command queued'
    ...
    async _executeCommand(commandItem) {
        ...
        this._appendLiveEvent(commandItem, {
            type: 'status',
    ...
    _appendLiveEvent(commandItem, event) {
        try {
            const { appendEvent } = require('../utils/live-stream');
            const sessionId = commandItem.sessionId;
            const token = commandItem.session?.token || null;
            appendEvent(sessionId, {
                ts: new Date().toISOString(),
                token,
                sessionId,
                source: this.runner.name || 'claude',
                ...event
            });
        } catch (error) {
            this.logger.warn('Failed to append live event:', error.message);
        }
    }
    ...
    _handleAdminCommand(command) {
        ...
        const liveMatch = normalized.match(/^live\s+([A-Z0-9]{6,8})$/i);
        if (liveMatch) {
            const token = liveMatch[1].toUpperCase();
            const { issueLiveSecret, getLiveBaseUrl } = require('../utils/live-stream');
            const session = require('../utils/session-store').findSessionByToken(token);
            if (!session) return 'Invalid token.';
            const baseUrl = getLiveBaseUrl();
            if (!baseUrl) return 'Live stream base URL is not configured.';
            const secret = issueLiveSecret(token);
            return `Live stream: ${baseUrl}/tokens/${token}/live/view?pw=${secret.liveSecret}`;
        }
