...
const webhookEnabled = (
    ...
    (process.env.LINE_ENABLED === 'true' && process.env.LINE_CHANNEL_ACCESS_TOKEN)
);

if (webhookEnabled) {
    console.log('🌐 Starting webhook master server...');
    const masterProcess = spawn('node', ['start-webhook-master.js'], {
        stdio: ['inherit', 'inherit', 'inherit'],
        env: process.env
    });

    masterProcess.on('exit', (code) => {
        console.log(`🌐 Webhook master server exited with code ${code}`);
    });

    processes.push({ name: 'Webhook Master', process: masterProcess });
}
...
console.log('📡 Live stream routes are mounted on the webhook master server.');
