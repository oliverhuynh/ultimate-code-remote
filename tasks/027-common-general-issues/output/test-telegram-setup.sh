...
if [ -z "$WEBHOOK_BASE_URL" ]; then
    echo "❌ WEBHOOK_BASE_URL not set in .env"
    exit 1
else
    echo "✅ WEBHOOK_BASE_URL found: $WEBHOOK_BASE_URL"
fi
...
if echo "$webhook_response" | grep -q "$WEBHOOK_BASE_URL"; then
    echo "✅ Webhook is correctly set"
else
    echo "⚠️  Webhook not set to expected URL"
    echo "Expected: $WEBHOOK_BASE_URL"
...
if pgrep -f "ngrok" > /dev/null; then
    ...
    echo "⚠️  ngrok not found - make sure to run: ngrok http ${WEBHOOK_PORT:-3001}"
fi

if pgrep -f "start-webhook-master" > /dev/null; then
    echo "✅ Webhook master service is running"
else
    echo "⚠️  Webhook master service not running - run: node start-webhook-master.js"
fi
...
echo ""
echo "Next steps:"
echo "1. Make sure ngrok is running: ngrok http ${WEBHOOK_PORT:-3001}"
echo "2. Make sure webhook service is running: node start-webhook-master.js"
